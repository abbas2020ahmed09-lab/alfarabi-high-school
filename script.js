...
const GITHUB_CONFIG = {
    repo: 'abbas2020ahmed09/alfarabi-high-school', // غير هذا لاسم المستودع الخاص بك
    branch: 'main',
    apiToken: 'ghp_your_personal_access_token_here', // يجب أن يكون آمناً - لا تضعه هنا مباشرة
    dataFile: 'data.json'
};
...
class GitHubSync {
    ...
    async uploadToGitHub(data) {
        if (!this.config.apiToken || this.config.apiToken === 'ghp_your_personal_access_token_here') {
            console.log('GitHub token not configured, using fallback');
        ...
        try {
            ...
            let sha = null;
            try {
                const getResponse = await fetch(`https://api.github.com/repos/${this.config.repo}/contents/${this.config.dataFile}?ref=${this.config.branch}`, {
                    headers: {
            ...
            const body = {
                message: `Update school data - ${new Date().toISOString()}`,
                content: content,
            ...
            const response = await fetch(`https://api.github.com/repos/${this.config.repo}/contents/${this.config.dataFile}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `token ${this.config.apiToken}`,
                    'Accept': 'application/vnd.github.v3+json',
            ...
            if (response.ok) {
                console.log('Data uploaded to GitHub successfully');
                showNotification('تم مزامنة البيانات مع جميع الأجهزة', 'success');
            } else {
...
function buildDeveloperDashboard() {
    ...
    <div class="dashboard developer-dashboard">
        ...
        <div class="dev-actions-grid">
            ...
            <button class="dev-action-btn" onclick="syncDataFromServer()">🔄 تحديث من الخادم</button>
...
function syncDataFromServer() {
    showNotification('جاري تحديث البيانات من الخادم...', 'info');
    gitHubSync.loadFromGitHub().then(success => {
        if (success) {
            // Reload all data
            loadInitialData();
            showNotification('تم تحديث البيانات من الخادم بنجاح', 'success');
        } else {
            showNotification('فشل في تحديث البيانات من الخادم', 'danger');
        }
    });
}
